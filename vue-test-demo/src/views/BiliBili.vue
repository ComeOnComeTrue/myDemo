  ...
  <div id="app" @scroll="handleScroll">
      ...
      <div class="header">
          ...
          <div class="search">
              ...
              <span class="search-input">北冥有鱼，化而为鹏,鹅鹅鹅鹅鹅鹅饿鹅鹅鹅饿呃呃呃呃呃呃呃呃</span>
          </div>
          <div class="avatar">
          ...
          <div class="download-logo">下载App</div>
      </div>
      ...
      <!-- 视频区域 -->
      <video-bilibili :videoList="videoList" />
      <!-- <div class="video-box">
        <ul class="video-list">
          <li 
...
<script>
import videoBilibili from "../components/bilibili/BilibiliVideoList"
export default {
  components: {
    videoBilibili,
  },
  ...
  methods: {
    ...
    autoMove() {
      setTimeout(() => {
        if(this.bannerActiveIndex === 0) { // 索引0 重新动画
    ...
    handleScroll (e) {
      ...
      if(toBottomHeight <= 200 && !this.videoGettingData) {
        ...
        this.$axios.get('/video',{
          params: { // 后台要求传递 
              start: videoLength, // 从哪开始获取
              offset: 12, // 获取数量
          }
        }).then(result => {
          this.videoOldList.push( ...result.data );
    ...
    getData () {
      this.$axios.all([
          ...
          this.$axios.get('/video',{
             params: { // 后台要求传递 
                start: this.videoOldList.length, // 从哪开始获取
                offset: 12, // 获取数量
              } 
          }),
      ]).then(this.$axios.spread((navRes, bannerRes, videoRes) => {
    ...
    initBannerList(bannerRes) {
      ...
      const lastImg = {...this.bannerList[0]};
      lastImg.id = Math.floor( Math.random() * 10000000 );
      this.bannerList = [...this.bannerList, lastImg];
    },
    initVideoList(videoRes) {
      this.videoCount = videoRes.count; // 总视频长度
